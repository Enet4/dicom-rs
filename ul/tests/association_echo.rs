use dicom_ul::{
    association::client::{self, ClientAssociationOptions},
    pdu::{self, Pdu, PDataValue, PresentationContextResult, PresentationContextResultReason},
};

use std::net::SocketAddr;
use rand::Rng;

use dicom_ul::association::server::{self, ServerAssociationOptions};
...
fn bogus_packet(len: usize) -> Pdu {
    Pdu::PData {
        data: vec!(PDataValue {
            ...
            value_type: pdu::PDataValueType::Command,
            is_last: true,
            data: vec![0_u8; len],
        }),
    }
    ...
    -> Result<(std::thread::JoinHandle<Result<()>>, SocketAddr)>
{
    ...
    let h = std::thread::spawn(move || -> Result<()> {
        ...
        match association.send(&packet) {
            Err(server::Error::SendTooLongPdu {..}) => (),
...
fn scu_scp_association_test() {
    ...
    match association.send(&packet) {
        Err(client::Error::SendTooLongPdu {..}) => (),
        e => panic!("Expected SendTooLongPdu but didn't happen: {:?}", e)
    }
